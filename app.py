...
# إعداد التطبيق
app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # حد أقصى لرفع الصور 16 ميجا

# تفعيل الـ CORS لضمان اتصال اللوحة والموقع بالسيرفر بدون قيود
CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=True)

# ربط قاعدة بيانات Neon Postgres - الرابط العالمي الحقيقي
...
# ---------------------------------------------------------
# الهيكل الإنشائي لقاعدة البيانات (Database Schema)
# ---------------------------------------------------------

# جدول المشاريع: لتخزين أعمالك وصورك
...
# جدول الرسائل: لاستقبال رسائل الزوار من الموقع
...
# جدول النبذة: لتعديل النص التعريفي عنك في الموقع من خلال اللوحة
class About(db.Model):
    __tablename__ = 'about'
    id = db.Column(db.Integer, primary_key=True)
    content = db.Column(db.Text)

# إنشاء الجداول في السيرفر فور تشغيله
with app.app_context():
    db.create_all()

# ---------------------------------------------------------
# العمليات التشغيلية (API Endpoints)
# ---------------------------------------------------------

# 1. نظام رفع الصور العالمي عبر ImgBB
...
# 2. إدارة المشاريع (إضافة وعرض وتعديل وحذف)
...
# 3. إدارة الرسائل (استقبال من الموقع وعرض في اللوحة)
...
def handle_messages():
    ...
    return jsonify([{"id":m.id,"name":m.name,"message":m.message,"date":m.created_at} for m in all_m])
...
# 4. إدارة النبذة الشخصية (About Me)
